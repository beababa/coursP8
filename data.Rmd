...
library(sqldf)
library(sf)
data <- read.csv("c:/Users/Etudiant/Downloads/table-adresses-reu.csv", nrows = 10)
str(data)
write.csv(data, "data/test.csv")
data <- read.csv.sql("c:/Users/Etudiant/Downloads/table-adresses-reu.csv", sql = "select * from file where code_commune_ref like '9329'")
data2 <- read.csv.sql("data/test.csv", sql = "select * from file where code_commune_ref = '9329'")
data2
```
...
#modif à la main et export carte umap


```{r}
zone <- st_read("data/secteurFinal.geojson")
joint <- zone
```
...
joint$txt <- paste(joint$numBureau,joint$prénom)
mf_label(joint, "txt", halo =T, cex = 0.7, overlap=F)
